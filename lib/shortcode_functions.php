...
function get_event_desc_shortcode(): string
{
    try {
        ...
        $msg = $e->getMessage();
        return "<div data-shortcode='event-desc' data-error='$msg'></div>";
    }
}
...
function get_event_book_now_shortcode(): string
{
    try {
        ...
        $msg = $e->getMessage();
        return "<div data-shortcode='" . Constants::SHORTCODE_BOOK_NOW . "' data-error='$msg'></div>";
    }
}
...
function get_event_book_now_general_admission_shortcode(): string
{
    try {
        ...
        $msg = $e->getMessage();
        return "<div data-shortcode='" . Constants::SHORTCODE_BOOK_GENERAL_ADMISSION . "' data-error='$msg'></div>";
    }
}
...
function get_museum_hours_shortcode(): string
{
    if (function_exists('get_museum_hours_shortcode_data')) {
        try {
            ...
            $msg = $e->getMessage();
            return "<div data-shortcode='museum-hours' data-error='$msg'></div>";
        }
    }

    //todo: need a fallback message in case this data is not available.
    $message = 'missing function get_museum_hours_shortcode_data()';
